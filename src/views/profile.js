...
class announceSubscriptionForm {
  ...
  view() {
    ...
    return m(
      'button',
      buttonArgs,
      user.get().send_newsletter ? 'unsubscribe from Newsletter' : 'subscribe to Newsletter'
    );
  }
}
    ...
    return m('div', [
      ...
      m(
        ...
        groups.getList().map(group => {
          ...
          return m('div', [m('span', group.name), m('button', buttonArgs, 'enroll')]);
        })
      ),
    ]);
  }
